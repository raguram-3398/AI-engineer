...
def test_check_confidence_clean():
    result = check_confidence("The customer was interested in our pricing.", request_id="r1")
    assert result.is_low_confidence is False


def test_check_confidence_flagged():
    """More than 30% consecutive repeated words triggers low-confidence flag."""
    result = check_confidence("the the the the customer customer was interested", request_id="r1")
...
async def test_transcribe_audio_timeout():
    mock_client = MagicMock()
    mock_client.audio.transcriptions.create = AsyncMock(side_effect=asyncio.TimeoutError())

    with patch("services.transcription._get_client", return_value=mock_client):
        with pytest.raises(TimeoutError, match="Whisper timed out"):
            await transcribe_audio(
                audio_bytes=b"fake_audio", filename="test.mp3",
                request_id="req-1", timeout=0.001,
            )